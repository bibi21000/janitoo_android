 ...
 */
package org.janitoo.paho.android.service.sample;
...
import org.janitoo.paho.android.service.sample.R;
import org.eclipse.paho.client.mqttv3.MqttConnectOptions;
import android.content.Context;
import android.text.Html;
import android.text.Spanned;
import org.json.*;
import org.janitoo.paho.android.service.MqttAndroidClient;
...
public class Connection {
  ...
  /** The history of the {@link MqttAndroidClient} represented by this <code>Connection</code> object **/
  private ArrayList<String> history_values_user = null;
  /** The history of the {@link MqttAndroidClient} represented by this <code>Connection</code> object **/
  private ArrayList<String> history_values_basic = null;
  ...
  public Connection(String clientHandle, String clientId, String host,
    ...
    history_values_user = new ArrayList<String>();
    history_values_basic = new ArrayList<String>();
  ...
  /**
   * Add an action to the history of the client
   * @param hadd the history item to add
   * @param uuid the history item to add
   * @param label the history item to add
   * @param value the history item to add
   * @param unit the history item to add
   */
  private void _addValueUser(String hadd, String uuid, String label, String value, String unit) {

    String payload = String.format("%-11s %-15s %-20s %-10s %-5s", hadd, uuid, label, value, unit);
    ...
    String timestamp = context.getString(R.string.timestamp, args);
    history_values_user.add(0, payload + timestamp);
    if(history_values_user.size()>=ActivityConstants.historyMaxEntries){
        history_values_user.remove(history_values_user.size()-1);
      }
  }

  /**
   * Add an action to the history of the client
   * @param payload the history item to add
   */
  public void addValueUser(String payload) {
    JSONObject obj = null;
    try {
      obj = new JSONObject(payload);
      if (obj.has("uuid")) {
        //single value
        this._addValueUser(obj.getString("hadd"), obj.getString("uuid"), obj.getString("label"), obj.getString("data"), obj.getString("units"));
      } else {
        //Multiples values
        Iterator<String> keys = obj.keys();
        while (keys.hasNext()) {
          JSONObject nobj = obj.getJSONObject((String) keys.next());
          this._addValueUser(nobj.getString("hadd"), nobj.getString("uuid"), nobj.getString("label"), nobj.getString("data"), nobj.getString("units"));
        }
      }
      notifyListeners(new PropertyChangeEvent(this, ActivityConstants.history_values_userProperty, null, null));
    }
     ...
     * @param unit the history item to add
     */
    private void _addValueBasic(String hadd, String uuid, String label, String value, String unit) {
        ...
        String timestamp = context.getString(R.string.timestamp, args);
        history_values_basic.add(0, payload + timestamp);
        if(history_values_basic.size()>=ActivityConstants.historyMaxEntries){
            history_values_basic.remove(history_values_basic.size()-1);
        }
    }
     ...
     * @param payload the history item to add
     */
    public void addValueBasic(String payload) {
        JSONObject obj = null;
        try {
            obj = new JSONObject(payload);
            if (obj.has("uuid")) {
                ...
                while (keys.hasNext()) {
                    JSONObject nobj = obj.getJSONObject((String) keys.next());
                    this._addValueBasic(nobj.getString("hadd"), nobj.getString("uuid"), nobj.getString("label"), nobj.getString("data"), nobj.getString("units"));
                }
            }
            notifyListeners(new PropertyChangeEvent(this, ActivityConstants.history_values_basicProperty, null, null));
        }
        catch (org.json.JSONException e)
        {
            e.printStackTrace();
   ...
   * @return an array of history entries
   */
  public Spanned[] history_values_basic() {

    int i = 0;
    Spanned[] array = new Spanned[history_values_basic.size()];

    for (String s : history_values_basic) {
      if (s != null) {
        array[i] = Html.fromHtml(s);
        i++;
      }
   ...
   * @return an array of history entries
   */
  public Spanned[] history_values_user() {

    int i = 0;
    Spanned[] array = new Spanned[history_values_user.size()];

    for (String s : history_values_user) {
      if (s != null) {
        array[i] = Html.fromHtml(s);
        i++;
      }
